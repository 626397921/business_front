...
// ========== 核心修改4：统一使用全局axios，优化请求逻辑 ==========
const loadCategoryList = async () => {
  try {
    tableLoading.value = true
    // 拼接全局baseURL + 接口路径
    const res = await $axios.get(`${BASE_API}/category/list/`)
    categoryList.value = res.data || [] // 适配main.js返回response.data的逻辑
  } catch (error) {
    ElMessage.error('加载类别列表失败，请重试')
  } finally {
...
const handleAddCategory = async () => {
  ...
  try {
    ...
    // 复用全局拦截器的错误提示，避免重复弹窗
    if (!error.message.includes('该类别已存在')) {
...
const handleAddMaterial = async () => {
  ...
  if (!category_id || !trimName || price <= 0) return ElMessage.warning('表单数据有误，请检查！')
  ...
  if (isDuplicate) {
    return ElMessage.warning('该类别下已存在相同名称的原材料，请勿重复添加')
  }
...
const handleEditMaterial = async () => {
  ...
  const trimName = name.trim()
  if (!material_id || !category_id || !trimName || price <= 0) return ElMessage.warning('数据有误！')
  ...
  if (isDuplicate) {
    return ElMessage.warning('该类别下已存在相同名称的原材料，请勿重复')
  }
...
const handleDelete = async (materialId) => {
  if (!materialId) return ElMessage.warning('无效的原材料ID')
  try {
